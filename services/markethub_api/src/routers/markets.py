...
async def api_market_trading_calendar(exchange: str = Query("SSE", description='交易所标识，如 `SSE`、`SZSE`、`BSE`、`HKEX`。'), start_date: str = Query("", description='起始日期，格式 `YYYY-MM-DD`。'), end_date: str = Query("", description='结束日期，格式 `YYYY-MM-DD`。'), is_open: bool | None = Query(None, description='是否为开市日。'), data_version: str = Query(..., min_length=1, description='`/api/health` 返回的市场数据版本；版本不匹配会拒绝读取。')) -> list[dict[str, object]]:
    return await run_data_task(_dump_item_list, markets.get_trading_calendar, (exchange, start_date, end_date, is_open, data_version))
