...
- `MHK_STORE_DB_CONCURRENCY`：store DB 并发上限，默认 `8`。
- `MHK_STORE_DB_RPS`：store DB 每秒请求上限，默认 `0`，表示不按 RPS 限流。
- `MHK_STORE_DB_POOL_SIZE`：store DB 连接池大小，默认 `8`。
- `MHK_PROVIDER_MAX_RETRIES`：provider 最大重试次数，默认 `2`。
- `MHK_TUSHARE_MAX_RETRIES`：TS 最大重试次数，未配置时使用 `MHK_PROVIDER_MAX_RETRIES`。
- `MHK_STORE_DB_MAX_RETRIES`：store DB 最大重试次数，默认 `0`。
...
- store DB 连接池由 QuoteMux 管理；连接池指标可通过诊断接口查看。
...
- `tushare`、`opentdx`、`efinance`、`mootdx`、`akshare`、`store_db` 的活跃数、排队数、总请求数、错误率、重试次数、限流等待次数和耗时。
- store DB 连接池大小、已创建连接数、活跃连接数、空闲连接数、复用次数和丢弃次数。
...
- 回滚配置：如果只需要放宽限制，可调大 `MHK_GLOBAL_CONCURRENCY`、`MHK_TUSHARE_CONCURRENCY`、`MHK_OPENTDX_CONCURRENCY` 和 `MHK_STORE_DB_POOL_SIZE`。
...
- 若 DB `created` 接近 `pool_size` 且 `active` 长期不下降，应检查慢 SQL 或 store DB 连接状态。
