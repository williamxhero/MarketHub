...
- 主路径先读取 QuoteMux Store 的 `stocks.quotes.daily_snapshot`，未命中时按 Capability Matrix 并发读取当日快照。
- Runtime Profile 会按 Capability Matrix 勾选的源并发读取当日快照，再按该 capability 的 `merge_strategy` 合并成一份结果。
- `fact.stock_daily_1d` 已纳入 `BJSE` 正式日线口径，所以该快照接口会返回北交所股票。
- 该接口只支持单个交易日快照，不支持区间查询；区间行情继续使用 `GET /api/stocks/quotes`。
