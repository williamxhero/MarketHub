...
def _run_market_migration() -> None:
    migration_path = MARKETHUB_ROOT / "scripts" / "migrate_market_daily_contracts.py"
    subprocess.run([sys.executable, str(migration_path)], cwd=str(WORKSPACE_ROOT), check=True)
